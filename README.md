...
**原因**：`care-wedo-app/public/_redirects` 與 `functions/[[path]].ts` catch-all 衝突，導致靜態 JS/CSS 被誤導向 `index.html`，React 無法掛載。

**已完成修復**：
- 刪除 `_redirects` 檔案
- 強化 `[[path]].ts`：靜態資源副檔名永遠直接回傳，不 fallback 到 `index.html`
- 新增 React Error Boundary（防止 render 錯誤導致白屏）
- `index.html` 加入 noscript fallback 與 loading 提示
...
- [x] ~~手機/電腦首頁空白問題~~：已修復（刪除 `_redirects`、強化 catch-all、加入 Error Boundary）
