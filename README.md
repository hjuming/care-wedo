...
- 2026-06-20 已補 `SUBSCRIPTION_STATE_MACHINE.md` 與 `functions/_shared/subscription_state.ts`，先定義 `beta`、`checkout_pending`、`active`、`past_due`、`grace_period`、`suspended`、`cancel_at_period_end`、`canceled`、side effects 與 webhook idempotency，並以 `functions/_tests/subscription-state.test.ts` 鎖定合法 / 非法 transition；尚未接正式付款按鈕、金流 provider 或 webhook。
...
- 商轉前置：後端已補 billing entitlement、event helper 與 pure subscription state machine，主帳號不列入共同協作者費用；正式收費前仍需補 migration 欄位、webhook fixture test、checkout API 與金流 webhook。
