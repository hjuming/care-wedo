  ...
  "appointments": [
    {
      ...
      "reminder_text": "照護對象，明天要去醫院看診，記得帶健保卡。"
    }
  ],
  "medications": [
    {
...
3. 領藥 reminder_text 範例：「照護對象，明天開始可以領下一次藥了，記得帶健保卡。」
4. 若為「檢驗及預約單」，請特別留意附註中有無看診前要做的事，如先量血壓、空腹、帶健保卡，務必放進 notes 或 reminder_text。
5. 藥名可照單子保留，但 purpose、warnings、reminder_text 要盡量白話。`;

const medicalTextParsePrompt = `${medicalDocParsePrompt}

補充情境：
- 這次使用者提供的是「文字內容」，不是照片。
- 文字可能來自 LINE 訊息、診所網站、掛號資訊、藥袋文字或家人轉貼的備忘。
- 請只提取照護提醒需要的資訊：預約、檢查、復健、看牙、領藥、吃藥與重要注意事項。
- 若文字中沒有明確日期或時間，不要硬編預約日期；可保留在 notes，但 appointments 請留空。
- 若文字包含診所介紹或醫師背景，請只留下與提醒有關的摘要，不要把完整介紹塞入 notes。
- 如果院所名稱已包含醫師姓名，doctor 欄位仍可填醫師姓名，但提醒摘要不要重複。`;
...
async function parseGeminiMedicalParts(env: Env, parts: Array<Record<string, unknown>>) {
...
export async function parseMedicalImages(env: Env, images: Array<{ data: string; media_type: string }>) {
  return parseGeminiMedicalParts(env, [
    { text: medicalDocParsePrompt },
    ...images.map(img => ({ inline_data: { mime_type: img.media_type, data: img.data } }))
  ]);
}

export async function parseMedicalText(env: Env, text: string) {
  const sourceText = cleanText(text);
  if (!sourceText) throw new Error("請貼上要整理的文字");

  return parseGeminiMedicalParts(env, [
    { text: `${medicalTextParsePrompt}\n\n使用者貼上的文字：\n${sourceText}` }
  ]);
}
