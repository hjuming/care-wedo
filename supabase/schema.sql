...
-- Data containment note (2026-06-20):
-- Cloudflare Pages Functions currently access Supabase REST with service_role.
-- service_role bypasses RLS, so these enabled RLS flags are not the active API
-- enforcement layer yet. Until explicit policies are added, tenant isolation is
-- enforced by Functions ownership filters plus functions/_tests/tenant-isolation.test.ts.
...
-- Defensive authenticated read policies (2026-06-20):
-- Direct writes remain closed to anon/authenticated. Cloudflare Functions still
-- use service_role and therefore bypass RLS; handler ownership filters remain
-- the active write boundary.
