...
> **最後更新**：2026-07-15
> **當前狀態**：LINE 長輩上傳流程已跑通；後台已收斂成協作者管理中心，並可查看去識別化提醒送達紀錄；Google OAuth 後台登入 MVP 已完成程式與 Phase 58 migration；protected data API 已改用 request-scoped `getRequestUser(context)`，deploy 前 CI gate 已上線；Care WEDO 綠界定期定額已完成小額正式付款實測、取消續扣、交易紀錄與付款返回補償。
...
| 付費方案金流 | ✅ 核心流程完成 | Care checkout、WEDOPR 中央 gateway、綠界定期定額、HMAC／CheckMac 驗證、server callback、付款返回補償、小額 live callback、停止下期續扣與交易紀錄已完成；減價換約、自助退款與邀請席位釋放仍待補強 |
...
| 跨戶隔離測試 | 已上線 | functions tenant-isolation 測試已覆蓋 medications、appointments、profiles、care_documents：跨群組 PATCH 回 403 / 404 且不寫入；同群組 PATCH 回 200；另覆蓋 appointment create foreign profile 不 insert、owned profile insert 帶 owned group/profile/user scope、medication taken mixed/foreign ids 不寫 log、owned medications 寫入 owned group logs、dashboard read、documents list、document detail linked records、foreign document signed URL 不外洩、foreign document 不觸發 Storage delete、upload storage path 使用 group/profile namespace |
...
## 6.1 2026-07-15 綠界金流完成紀錄

本日完成 Care WEDO 與 WEDOPR 中央金流的正式綠界定期定額流程驗收：

1. 以真實信用卡完成小額付款與手機驗證，確認綠界交易成功通知信可收到。
2. 修正付款返回後只停留在結果頁、但新增照護對象／協作者邀請沒有套用的問題；現在會以 request id 查詢 webhook 入帳狀態，再補執行待辦動作。
3. 新增停止下期續扣：Care 先驗證管理者／付款負責人權限，中央 gateway 成功呼叫 ECPay Period Action `Cancel` 後，才寫入 `cancel_at_period_end`。
4. 新增群組範圍交易紀錄查詢，回傳帳單與 provider 事件，不回傳卡號或 secret。
5. 月費增加採新合約換約：新付款成功後由中央 gateway 取消舊定期定額；新付款失敗時保留舊合約。
6. 月費減少暫不允許靜默修改，避免綠界仍按原合約金額扣款；後續以「本期結束後重新授權較低金額」設計。
7. Phase 62 將照護對象資料庫備援預設從「親愛的爸爸 / 媽媽」改為「照護對象」；正常流程仍優先使用註冊者名稱與頭像。
8. Phase 63 補上 provider reference、取消狀態、取消原因與帳務索引。

驗證證據：Care Functions 59/59、Care 前端 198/198、WEDOPR ECPay 7/7、TypeScript、ESLint、stylelint 全部通過。詳細交接見 [`CARE_WEDO_ECPAY_BILLING_IMPLEMENTATION.md`](./CARE_WEDO_ECPAY_BILLING_IMPLEMENTATION.md)。
...
| PR-CARE-P0-006 | P0 | Data Containment Contract | 🟡 基礎完成：已補 `DATA_CONTAINMENT_CONTRACT.md`，明文採 service-role-only + app-layer ownership filters；tenant-isolation 已覆蓋四個核心 PATCH、appointment create、medication taken、dashboard/documents 讀取 scope、foreign document signed URL / storage delete 阻擋與 upload path namespace；已補 authenticated read-only table / Storage object policies 與 storage smoke，待補 staging Google E2E、Storage policy live verification 與 direct-write RLS 設計 |
...
| PR-CARE-P1-002 | P1 | Billing Data Foundation | ✅ 核心流程完成：已完成 `billing_events`、`billing_subscriptions`、`invoices`、provider reference、取消狀態與歷史查詢；Care checkout、付款 UI、WEDOPR 中央 gateway、live callback 與取消續扣已驗收；減價換約、自助退款與邀請席位釋放列為後續工作 |
...
- 補資料圍堵第二層防護：目前已明文採 service-role-only + app-layer ownership filters，且 medications / appointments / profiles / care_documents PATCH、appointment create、medication taken、dashboard/documents 讀取 scope、foreign document signed URL、foreign document storage delete、upload path namespace 都有負向或合約測試；DB 層已補 authenticated read-only table / Storage object policies、direct write revoke 與 storage smoke。下一步是 staging Google E2E、Storage policy live verification 與 direct-write RLS 設計。
...
- Care checkout API、付款 UI、live callback 與停止下期續扣已完成；部署前需逐環境核對 migration、Cloudflare secrets 與部署後 smoke。
- 方案升級已採「新訂閱成功後取消舊訂閱」；方案降級目前阻擋靜默變更，下一階段補週期到期換約流程。
- 退款／退刷與取消續扣分開處理；目前退款由商戶後台或獨立人工流程處理，不對一般使用者開放自助退款。
- 邀請協作者目前會先保留計費席位；後續補邀請過期／未接受時釋放席位。
- 完整技術紀錄、migration、環境變數、測試證據與其他 WEDO 專案複用方式見 [`CARE_WEDO_ECPAY_BILLING_IMPLEMENTATION.md`](./CARE_WEDO_ECPAY_BILLING_IMPLEMENTATION.md)。
- 資料匯出與家庭交接。
- 醫療免責聲明與法務再檢查。
...
- `npm run test:functions`：27/27 passed。
