...
| 付費方案金流 | 🟡 基礎完成 | 已先定 subscription state machine，補 Care 端 `POST /api/billing/webhook` 驗 WEDOPR HMAC、provider event 去重與 fixture tests；正式公開收費前仍需補 Care checkout API / 付款 UI 與小額 live callback 驗收 |
...
| PR-CARE-P1-002 | P1 | Billing Data Foundation | 🟡 基礎完成：已完成 `billing_events`、`billing_subscriptions`、`invoices` migration 套用，`/api/groups` 回傳 `billing_entitlement` 並被設定頁作為主要來源；已補 `SUBSCRIPTION_STATE_MACHINE.md`、pure transition helper、Care 端中央金流 webhook 與 fixture tests；待補 checkout API / 付款 UI / live callback 驗收 |
...
- Care checkout API / 付款 UI / live callback 驗收。
- 方案升級 / 降級。
- Billing schema、entitlement helper、paid action event、draft invoice snapshot、`SUBSCRIPTION_STATE_MACHINE.md`、pure transition helper 與 Care 端中央金流 webhook 已有基礎；下一步補 Care checkout API、付款 UI、小額 live callback 驗收，再把取消、退款與付款失敗寬限期完整接入 `billing_events` / `invoices`。
- 資料匯出與家庭交接。
- 醫療免責聲明與法務再檢查。
