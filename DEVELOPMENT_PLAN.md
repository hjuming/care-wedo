...
| 商轉資料基礎 | 🟡 基礎完成 | 已補 billing schema migration、後端 entitlement helper、paid action event 與 draft invoice snapshot；production Supabase 已套用 phase55 billing migration，金流 webhook 尚未接 |
...
| PR-CARE-P1-002 | P1 | Billing Data Foundation | 🟡 基礎完成：已補 `billing_events`、`billing_subscriptions`、`invoices` migration 草案、RLS/service-role grant、`resolveGroupBillingEntitlement`、`recordBillingGroupEvent`；新增照護對象與新協作者加入會寫入事件、subscription snapshot 與 draft invoice。待套 production migration、升降級/取消政策與金流 webhook |
