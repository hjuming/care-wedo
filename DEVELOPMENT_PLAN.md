...
> **最後更新**：2026-06-20
> **當前狀態**：LINE 長輩上傳流程已跑通；後台已收斂成協作者管理中心，並可查看去識別化提醒送達紀錄；Google OAuth 後台登入 MVP 已完成程式與 Phase 58 migration；protected data API 已統一到 `getAuthenticatedUser()`，deploy 前 CI gate 已上線。
...
| Google OAuth 後台登入 | 🟡 基礎完成 | Supabase Auth token 與 LINE session 已統一到 `verifyCareIdentity`；前端已有 Google 登入與 `/auth/callback`；protected data API 已統一使用 `getAuthenticatedUser()`；待 production 套用 Phase 58 並設定 Google provider / redirect URL |
...
| API 身份與租戶隔離 | ✅ 已完成 | 12 個 protected data API 已統一走 `getAuthenticatedUser()`；新增 source guard 禁止 protected route 直接使用 LINE-only verifier；functions tenant-isolation 測試覆蓋跨群組 403 與同群組 200 |
| Reminder 發送模式 | ✅ 已完成 | `REMINDER_TEST_ONLY` 改為 `=== "1"` 才進測試模式；未設定或 `0` 為正式模式；cron log 會記錄 `test_only` |
| CI/CD 部署 gate | ✅ 已完成 | `deploy.yml` 部署前跑 lint、前端測試、functions tenant-isolation、receipt-pack、build；GitHub Actions 升至 `checkout@v7` / `setup-node@v6` |
| 真實單據回歸包 | 🟡 基礎完成 | 已建立 10 張去識別化 manifest、私有圖片目錄規則、validator、runbook 與 dry-run smoke runner；待補私有圖片 hash 與 LINE WebView 實測紀錄 |
| 藥品進階去重 | 🟡 未完成 | 目前仍主要依完整藥名比對 |
| 商轉資料基礎 | 🟡 基礎完成 | 已補 billing schema migration、後端 entitlement helper、paid action event 與 draft invoice snapshot；`/api/groups` 已回傳 `billing_entitlement`，主控台以後端快照顯示，Production 已套用 phase55。 |
...
| 項目 | 狀態 | 實作與驗收 |
|---|---|---|
| Auth 統一 | 已上線 | OCR、新增/編輯預約、用藥確認、文件、profile 排序等 protected data API 統一走 `getAuthenticatedUser()`；保留 LINE session / handoff / callback 的 LINE-only 驗證路徑 |
| 跨戶隔離測試 | 已上線 | 新增 functions tenant-isolation 測試：跨群組藥物 PATCH 回 403 且不寫入；同群組 PATCH 回 200 |
...
| Deploy gate | 已上線 | `main` push 後，Cloudflare Pages deploy 前必跑 lint、前端測試、functions tenant-isolation、receipt-pack、build |
| GitHub Actions runtime | 已上線 | `actions/checkout@v7`、`actions/setup-node@v6`；專案 Node 版本維持 22 |

剩餘風險：

- 仍需用 staging Google 帳號實測 OCR、新增預約、用藥確認三條寫入路徑。
- Production Cloudflare env 需維持 `REMINDER_TEST_ONLY` 不存在或為 `0`；只有明確測試時才設 `1`。
- `_middleware.ts` 與 handler 目前仍可能重複驗證身份；若要優化，下一步可讓 middleware 將解析後的 `userId` 傳給 handler，避免同一請求重複驗證。
...
| PR-CARE-P0-003 | P0 | Real Receipt Regression Pack | 🟡 基礎完成：已建立 10 張去識別化 manifest、私有圖片目錄規則、validator、runbook 與 dry-run smoke runner；待補真實圖片 hash 與 LINE WebView 實測紀錄 |
...
| PR-CARE-P0-005 | P0 | Google/Supabase Protected Write E2E | 🟡 待驗收：staging 以 Google 帳號實測 OCR、新增預約、用藥確認三條寫入路徑 |
...
- 用 staging Google 帳號實測 OCR、新增預約、用藥確認三條 protected write path；確認 Google/Supabase 使用者解析到自己的 `userId`，不落到 LINE-only 或共用測試帳號。
- 用至少 10 張真實台灣醫院單據測試 LINE 上傳流程：已建立 `test-fixtures/real-receipt-regression/manifest.json`、`REAL_RECEIPT_REGRESSION_RUNBOOK.md` 與 `npm run receipt-pack:smoke`，真實圖檔不進 Git。
...
- NewebPay / ECPay 金流。
- 方案升級 / 降級。
- Billing schema、entitlement helper、paid action event 與 draft invoice snapshot 已有基礎；下一步把升級、降級、取消、付款失敗寬限期與金流 webhook 接入 `billing_events` / `invoices`。
- 資料匯出與家庭交接。
- 醫療免責聲明與法務再檢查。
...
- `npm test`（`care-wedo-app`）：162/162 passed。
- `npm run lint`（`care-wedo-app`）：passed。
- `npm run build`（`care-wedo-app`）：passed。
- `npm run test:functions`：2/2 passed。
- `npm run receipt-pack:check`：OK。
...
- P1-002 billing foundation 基礎：已補 phase 55 migration、RLS/service-role grant、entitlement helper、paid action event、draft invoice snapshot 與回歸測試
