...
test("landing page copy matches elder-friendly beta positioning", () => {
  ...
  assert.match(app, /看診、用藥/);
  assert.match(app, /與照護提醒/);
  assert.match(app, /拍下藥袋、處方箋或預約單/);
  assert.match(app, /看第一次怎麼用/);
  assert.match(app, /ProductPreviewPanel/);
  assert.match(app, /今日照護/);
  ...
  assert.doesNotMatch(app, /Family Pro/);
